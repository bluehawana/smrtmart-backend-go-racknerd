...
echo -e "${GREEN}================================================${NC}"
echo -e "${GREEN}    SmrtMart Secure VPS Deployment${NC}"
echo -e "${GREEN}================================================${NC}"

# Check SSH key authentication
if ! ssh -o BatchMode=yes -o StrictHostKeyChecking=no "$VPS_USER@$VPS_HOST" "echo 'test'" &>/dev/null; then
    echo -e "${RED}Error: SSH key authentication is not set up!${NC}"
    echo "Please run ./setup-ssh-keys.sh first"
    exit 1
fi
...
SERVICE_STATUS=$(ssh "$VPS_USER@$VPS_HOST" "systemctl is-active $SERVICE_NAME")
